...
// Disassembler output helper
static char *disasm_out = NULL;
static size_t disasm_len = 0;

static void disasm_printf(const char *fmt, ...) {
    if (!disasm_out || disasm_len == 0) {
        return;
    }
    va_list ap;
    va_start(ap, fmt);
    int written = vsnprintf(disasm_out, disasm_len, fmt, ap);
    va_end(ap);
    if (written < 0) {
        return;
    }
